...
//    }
    ...
    public List<Employee> findAllLastPage() {
        ...
        int lastPageNum = (int) Math.ceil(countResult / size) ;//numero mas bajo

        // recuperar empleados de la última página
        Query query = session.createQuery("from Employee");
        query.setFirstResult((lastPageNum - 1) * size);//pocision de partida
        query.setMaxResults(size);                     // numero maximo que tiene que recuperar de empleados
