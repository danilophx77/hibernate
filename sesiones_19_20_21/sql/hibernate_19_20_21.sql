...
WITH cte_seats_archive_aircraft AS (
	DELETE FROM seats
...
INSERT INTO ob_users(email, passw) VALUES //para codificar
('user3@company.com', pgp_sym_encrypt('admin', 'secret')),
('user4@company.com', pgp_sym_encrypt('1234', 'secret'))

select id, email, pgp_sym_decrypt(passw::bytea, 'secret') as passw FROM ob_users;//para decodificar
...
$$
LANGUAGE plpgsql;
-- este ejemplo requiere mucho mas mantenimiento
...
--Por lo tanto, es importante diseñar y configurar los triggers de manera
--adecuada para minimizar su impacto en el rendimiento y garantizar su fiabilidad
--y mantenibilidad a largo plazo. También es importante documentar adecuadamente
--los triggers para que otros desarrolladores puedan entender y modificar su funcionamiento
--si es necesario.
